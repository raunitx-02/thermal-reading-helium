...
exports.login = async (req, res, next) => {
  try {
    ...
    await db.prepare('INSERT INTO refresh_tokens (id, user_id, token, expires_at, created_at) VALUES (?, ?, ?, ?, ?)').run(uuidv4(), user.id, refreshToken, expiresAt, Math.floor(Date.now() / 1000));
    await db.prepare('UPDATE users SET last_login = ? WHERE id = ?').run(Math.floor(Date.now() / 1000), user.id);
...
exports.refresh = async (req, res, next) => {
  try {
    ...
    const db = getDb();
    const stored = await db.prepare('SELECT * FROM refresh_tokens WHERE token = ? AND user_id = ? AND expires_at > ?').get(refreshToken, decoded.userId, Math.floor(Date.now() / 1000));
    if (!stored) return res.status(401).json({ success: false, message: 'Invalid or expired refresh token' });

    const user = await db.prepare('SELECT id, role FROM users WHERE id = ? AND is_active = 1').get(decoded.userId);
    if (!user) return res.status(401).json({ success: false, message: 'User not found' });

    const { accessToken, refreshToken: newRefreshToken } = generateTokens(user.id);
    const expiresAt = Math.floor(Date.now() / 1000) + 7 * 24 * 3600;
    await db.prepare('DELETE FROM refresh_tokens WHERE id = ?').run(stored.id);
    await db.prepare('INSERT INTO refresh_tokens (id, user_id, token, expires_at, created_at) VALUES (?, ?, ?, ?, ?)').run(uuidv4(), user.id, newRefreshToken, expiresAt, Math.floor(Date.now() / 1000));
...
exports.logout = async (req, res, next) => {
  try {
    const { refreshToken } = req.body;
    const db = getDb();
    if (refreshToken) await db.prepare('DELETE FROM refresh_tokens WHERE token = ?').run(refreshToken);
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    await db.prepare('UPDATE users SET reset_otp = ?, reset_otp_expires = ? WHERE id = ?').run(otp, expiresAt, user.id);
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    const db = getDb();
    const user = await db.prepare('SELECT * FROM users WHERE email = ? AND reset_otp = ? AND reset_otp_expires > ?').get(email.toLowerCase().trim(), otp, Math.floor(Date.now() / 1000));
    if (!user) return res.status(400).json({ success: false, message: 'Invalid or expired OTP' });

    const hash = await bcrypt.hash(newPassword, 12);
    await db.prepare('UPDATE users SET password_hash = ?, reset_otp = NULL, reset_otp_expires = NULL WHERE id = ?').run(hash, user.id);
