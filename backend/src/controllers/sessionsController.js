...
exports.createSession = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'ground_engineer') {
      const assigned = await db.prepare('SELECT id FROM assignments WHERE train_id = ? AND ground_engineer_id = ? AND status = "assigned"').get(train_id, req.user.id);
      if (!assigned) {
...
exports.saveReading = async (req, res, next) => {
  try {
    ...
    const db = getDb();
    const session = await db.prepare('SELECT status FROM inspection_sessions WHERE id = ?').get(session_id);
    if (!session) return res.status(404).json({ success: false, message: 'Session not found' });
    ...
    const existing = await db.prepare('SELECT id FROM thermal_readings WHERE session_id = ? AND zone_id = ?').get(session_id, zone_id);
    ...
    await db.prepare('UPDATE inspection_sessions SET updated_at = ? WHERE id = ?').run(now, session_id);
    
    // Handle Alert Generation (only insert alert if status is critical, and not already reported)
    if (status !== 'normal') {
      const existingAlert = await db.prepare('SELECT id FROM alerts WHERE reading_id = ?').get(readingId);
      if (!existingAlert) {
        ...
        const admins = await db.prepare("SELECT id FROM users WHERE role IN ('super_admin', 'branch_admin')").all();
      ...
      await db.prepare('DELETE FROM alerts WHERE reading_id = ? AND is_acknowledged = 0').run(readingId);
    }
...
exports.submitSession = async (req, res, next) => {
  try {
    ...
    const session = await db.prepare('SELECT * FROM inspection_sessions WHERE id = ?').get(id);
    if (!session) return res.status(404).json({ success: false, message: 'Session not found' });
    ...
    const record = await db.prepare('SELECT * FROM kpi_records WHERE inspector_id = ? AND date = ?').get(session.inspector_id, dateStr);
    
    // Count alerts/violations found in this session
    const violationsCount = (await db.prepare(`
