...
exports.create = async (req, res, next) => {
  try {
    ...
    const user = await db.prepare('SELECT id, role FROM users WHERE id = ?').get(ground_engineer_id);
    if (!user || user.role !== 'ground_engineer') {
      return res.status(404).json({ success: false, message: 'Invalid Ground Engineer' });
    }

    await db.prepare(`INSERT INTO assignments (id, train_id, ground_engineer_id, supervisor_id, status, assigned_at) VALUES (?, ?, ?, ?, 'assigned', ?)`)
