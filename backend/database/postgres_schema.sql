-- PostgreSQL Schema for Bogie Thermal Inspection Portal (Supabase)

-- Drop existing functions/tables if necessary
DROP FUNCTION IF EXISTS unixepoch() CASCADE;
DROP FUNCTION IF EXISTS date(bigint, text) CASCADE;
...
CREATE OR REPLACE FUNCTION date(epoch bigint, format text) RETURNS text AS $$
BEGIN
  IF format = 'unixepoch' THEN
    RETURN to_char(to_timestamp(epoch), 'YYYY-MM-DD');
  END IF;
  RETURN to_char(to_timestamp(epoch), 'YYYY-MM-DD');
END;
$$ LANGUAGE plpgsql;
